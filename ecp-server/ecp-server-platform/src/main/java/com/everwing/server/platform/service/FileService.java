package com.everwing.server.platform.service;
...
@Component
public class FileService extends BaseApiController {
	@Value("${platform.file.save.path}")
	 ...
	 * @description 上传文件到本地
	 */
	public UploadFile upload(MultipartFile multipartFile, String fileId,String accountId) {
		// 插入文件数据
		...
		saveFile.setUploadFileId(StringUtils.isBlank(fileId) ? UUID.randomUUID().toString() : fileId);
		...
		saveFile.setAccountId(accountId);
		checkSuccess(commonQueryApi.insertSelective(saveFile));
