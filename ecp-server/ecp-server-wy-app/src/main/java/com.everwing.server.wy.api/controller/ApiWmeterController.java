 ...
 */
@RestController
@RequestMapping("{version}/wmeter")
public class ApiWmeterController {

    private static final Logger logger= LogManager.getLogger(ApiWmeterController.class);
    ...
    public BaseApiVo<List<MeterTaskVo<MeterDataVo>>> queryTaskByCurrMonth(HttpServletRequest request, String pageNo, String pageSize) {
        ...
        logger.debug("根据用户查询当月所有水表抄表任务以及上月抄表数据," +
        ...
        logger.debug("根据用户查询当月所有水表抄表任务,远程调用返回数据：{}",remoteModelResult);
        ...
        if(remoteModelResult.isSuccess()) {
            ...
            logger.debug("根据taskId集合查询对应的水表数据，远程调用返回数据：{}",remoteModelResult2);
            if(remoteModelResult2.isSuccess()) {
                List<TcMeterData> meterDataList = remoteModelResult2.getModel();
                WmeterDataVo wmeterDataVo;
                for (TcMeterData meterData : meterDataList) {
                    wmeterDataVo = new WmeterDataVo();
                    wmeterDataVo.setMeterName(meterData.getMeterName());
                    wmeterDataVo.setMeterCode(meterData.getMeterCode());
                    wmeterDataVo.setLastMonthData(String.valueOf(meterData.getLastTotalReading()));
                    idsMap.get(meterData.getTaskId()).add(wmeterDataVo);
                }
                ...
                logger.debug("根据taskId集合查询对应的水表抄表数据，查询失败!");
            }
            ...
            logger.debug("根据用户查询当月所有水表抄表任务,查询失败!");
            returnApiVo.setCode(remoteModelResult.getCode());
            returnApiVo.setMessage(remoteModelResult.getMsg());
        }
        logger.debug("根据用户查询当月所有水表抄表任务以及上月抄表数据，返回给前端：{}",returnApiVo);
        return returnApiVo;
    }
    ...
    public BaseApiVo<List<MeterTaskVo>> queryHistoryTask(HttpServletRequest request, String pageNo, String pageSize) {
        ...
        logger.debug("根据用户查询水表历史任务数据,远程调用返回数据：{}",remoteModelResult);
        ...
        if(remoteModelResult.isSuccess()) {
            ...
            logger.debug("根据用户查询水表历史任务数据,查询失败!");
            returnApiVo.setCode(remoteModelResult.getCode());
            returnApiVo.setMessage(remoteModelResult.getMsg());
        }
        logger.debug("根据用户查询水表历史任务数据，返回给前端：{}",returnApiVo);
        return returnApiVo;
    }
    ...
    public BaseApiVo<List<MeterDataVo>> queryByTaskId(HttpServletRequest request,String taskId) {
        ...
        logger.debug("根据任务ID查询对应的水表数据,远程调用返回数据：{}",remoteModelResult);
        ...
        if (remoteModelResult.isSuccess()) {
            ...
            for (TcMeterData meterData : meterDataList) {
                ...
                wmeterDataVo.setMeterCode(meterData.getMeterCode());
                wmeterDataVo.setLastMonthData(String.valueOf(meterData.getLastTotalReading()));
                wmeterDataVo.setTotalReading(String.valueOf(meterData.getTotalReading()));
                meterDataVoList.add(wmeterDataVo);
            }
            ...
            logger.debug("根据任务ID查询对应的水表数据,查询失败!");
            returnApiVo.setCode(remoteModelResult.getCode());
            returnApiVo.setMessage(remoteModelResult.getMsg());
        }
        logger.debug("根据任务ID查询对应的水表数据，返回给前端：{}",returnApiVo);
        return returnApiVo;
    }
    ...
    public BaseApiVo<List<MeterDataVo>> queryByDescription(HttpServletRequest request,String description) {
        ...
        logger.debug("根据水表描述查询对应的水表数据,远程调用返回数据：{}",remoteModelResult);
        ...
        if (remoteModelResult.isSuccess()) {
            ...
            logger.debug("根据水表描述查询对应的水表数据,查询失败!");
            returnApiVo.setCode(remoteModelResult.getCode());
            returnApiVo.setMessage(remoteModelResult.getMsg());
        }
        logger.debug("根据水表描述查询对应的水表数据，返回给前端：{}",returnApiVo);
        return returnApiVo;
    }
    ...
    public BaseApiVo<List<MeterYearDataVo>> queryYearDate(HttpServletRequest request,String meterCode,String year) {
        ...
        logger.debug("根据年份查询对应的水表流水数据,远程调用返回数据：{}",remoteModelResult);
        ...
        if (remoteModelResult.isSuccess()) {
            ...
            for (TcMeterData meterData : tcMeterDataList) {
                ...
                meterYearDataVo.setCorrection(String.valueOf(meterData.getCorrection()));
            ...
            logger.debug("根据年份查询对应的水表流水数据,查询失败!");
            returnApiVo.setCode(remoteModelResult.getCode());
            returnApiVo.setMessage(remoteModelResult.getMsg());
        }
        logger.debug("根据年份查询对应的水表流水数据，返回给前端：{}",returnApiVo);
        return returnApiVo;
    }
    ...
    @ApiVersion(1.0)
    public BaseApiVo modifyData(HttpServletRequest request, String taskId, String meterCode, String totalReading, String fileId) {
        Account account= JSON.parseObject(request.getAttribute("account").toString(),Account.class);
        String accountId = account.getAccountId();
        String companyId = account.getCompanyId();
        int meterType = 0;
        ...
        RemoteModelResult<Integer> remoteModelResult = tcMeterDataApi.modifyWMeterData(companyId,taskId,accountId,meterCode,totalReading,fileId);
        logger.debug("修改水表抄表数据,远程调用返回数据：{}",remoteModelResult);

        if(!remoteModelResult.isSuccess()) {
            logger.debug("修改水表抄表数据失败!");
        }
