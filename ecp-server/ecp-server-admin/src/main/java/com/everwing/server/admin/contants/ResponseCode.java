package com.everwing.server.admin.contants;
...
public enum ResponseCode {
	...
	private static Map<String, ResponseCode> codeMap = new HashMap<String, ResponseCode>();
	...
	public static ResponseCode getByMapCode(String mapCode){
		if (codeMap.size() == 0) {
			for (ResponseCode rc : ResponseCode.values()) {
				for (String code : rc.getMapCode()) {
					codeMap.put(code, rc);
				}
			}
		}
		return codeMap.get(mapCode);
	}
