        ...
        return this.each(function() {
            ...
            this.createHtml = function() {
                ...
                if (para.dragDrop) {
                    ...
                    html += '				<div id="status_info" class="info">选中0张文件，共0B。上传不得超过5Mjpg、png的图片。</div>';
                    html += '				<div class="btns">';
                    ...
                    if (newStr.split(".")[0] != null) {
                        ...
                        if (jQuery.inArray(type, para.fileType) > -1) {
                            if (file.size >= para.fileSize) {
                                layer.alert('您这个"' + file.name + '"文件大小过大',{icon : 0});

                            } else {
                                arrFiles.push(file)
                            }
                        } else {
                            layer.alert('您这个"' + file.name + '"上传类型不符合',{icon : 0});

                        }
                    } else {
                        layer.alert('您这个"' + file.name + '"没有类型, 无法识别',{icon : 0});
            ...
            this.funDisposePreviewHtml = function(file, e) {
                ...
                var newStr = file.name.split("").reverse().join("");
                var type = newStr.split(".")[0].split("").reverse().join("");
                var fileImgSrc = "js/skins/images/fileType/";
                if (type == "rar") {
            ...
            this.addEvent = function() {
                ...
                $(".upload_btn").bind("click", function(e) {
                    if (ZYFILE.funReturnNeedFiles().length > 0) {
                        ...
                        layer.alert('请先选中文件再点击上传',{icon : 0});
                    }
                });
