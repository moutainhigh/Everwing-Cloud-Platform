 ...
 * @create 2018/6/5
 */
@JobHandler("waterAutoBillingJobHandler")
@Component
public class WaterAutoBillingJobHandler extends IJobHandler {
    ...
    public ReturnT<String> execute(String s) throws Exception {
        ...
        XxlJobLogger.log("自动任务 [水费自动计费] : 自动扫描开始.");
        for(Company company:companyList){
            String companyId=company.getCompanyId();
            XxlJobLogger.log("自动任务 [水费自动计费]{0}公司",companyId);
            waterElectAutoBillingService.autoBilling(companyId, BillingEnum.ACCOUNT_TYPE_WATER.getIntV());
            XxlJobLogger.log("自动任务 [水费自动计费]{0}公司结束",companyId);
        }
        XxlJobLogger.log("自动任务 [水费自动计费] : 结束.");
