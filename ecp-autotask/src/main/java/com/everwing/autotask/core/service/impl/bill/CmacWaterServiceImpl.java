...
import com.everwing.autotask.core.service.bill.CmacWaterService;
...
@Log4j2
@Service
public class CmacWaterServiceImpl extends BaseCmacBillingServiceImpl implements CmacWaterService{
    ...
    public void invoke(String companyId, TBsProject project) {
        ...
        if(CommonUtils.isEmpty(totals)){
            log.info("通用账户自动扣费 -> 扣费项 [水费] : 未找到可扣费的本周期水费总账单. 查询参数: {}, 进入下一计费项. ", paramTotal.toString());
            ...
            if(CommonUtils.isEmpty(histories)){
                log.info("通用账户自动扣费 -> 扣费项 [水费] : 未找到可扣费的本周期分账单. 总账单数据: {}, 进入下一计费项. ", total.toString());
                goNext(companyId, project);
                return;
            }
            ...
            for(TBsChargeBillHistory history : histories){
            	log.info("开始通用抵扣水费循环扣费{}",history.getBuildingCode());
                double zfje = 0.0;
                ...
                if(null == commonAccount){
                    ...
                    if(commonAccount.getAccountBalance() <= 0){
                        ...
                        if(!ohs.isEmpty()){
                            ...
                            if(zfLateFee > 0) {
                                ...
                                insertStreamList.add(new TBsAssetAccountStream(CommonUtils.getUUID(),commonAccount.getId(),-zfLateFee,
										new Date(),"system","system","通用账户抵扣水费违约金"));
                        ...
                        if( zfje > 0 ) {
                        	log.info("通用对水费进行了抵扣{}",account.getId());
							...
							insertStreamList.add(new TBsAssetAccountStream(CommonUtils.getUUID(),commonAccount.getId(),-zfje,
									new Date(),"system","system","通用账户抵扣水费本期欠费"));
                       ...
                       /* double data = CommonUtils.null2Double((currBalance >= currBillAmount) ? -currBillAmount : -currBalance);
                        ...
                        if(data != 0){
                            TBsAssetAccountStream commonStream=new TBsAssetAccountStream(commonAccount.getId(), data, StreamEnum.purpose_dk_to_water.getV());
                            insertStreamList.add(commonStream);  //通用账户的负流水 , 抵扣水费
