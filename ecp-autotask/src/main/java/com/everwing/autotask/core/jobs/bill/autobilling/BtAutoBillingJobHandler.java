 ...
 * @create 2018/6/5
 */
@JobHandler("btAutoBillingJobHandler")
@Component
public class BtAutoBillingJobHandler extends IJobHandler{
    ...
    public ReturnT<String> execute(String s) throws Exception {
        ...
        XxlJobLogger.log("自动任务 [本体基金自动计费] : 自动扫描开始.");
        for(Company company:companyList){
            String companyId=company.getCompanyId();
            XxlJobLogger.log("自动任务 [本体基金自动计费]{0}公司",companyId);
            wyAutoBillingService.autoBilling(companyId, BillingEnum.ACCOUNT_TYPE_BT.getIntV());
            XxlJobLogger.log("自动任务 [本体基金自动计费]{0}公司结束",companyId);
        }
