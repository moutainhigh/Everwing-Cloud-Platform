...
import com.everwing.autotask.core.service.bill.CmacElectService;
...
@Log4j2
@Service
public class CmacElectServiceImpl extends BaseCmacBillingServiceImpl implements CmacElectService{
    ...
    public void invoke(String companyId, TBsProject project) {
        ...
        if(CommonUtils.isEmpty(totals)){
            log.info("通用账户自动扣费 -> 扣费项 [电费] : 未找到可扣费的本周期电费总账单. 查询参数: {}, 进入下一计费项. ", paramTotal.toString());
            ...
            if(CommonUtils.isEmpty(histories)){
                log.info("通用账户自动扣费 -> 扣费项 [电费] : 未找到可扣费的本周期分账单. 总账单数据: {}, 进入下一计费项. ", total.toString());
                goNext(companyId, project);
                return;
            }
            ...
            for(TBsChargeBillHistory history : histories){
            	log.info("开始通用抵扣电费循环扣费{}",history.getBuildingCode());
                double zfje = 0.0;
                double zfLateFee = 0.0;
                double zfLateAmout = 0.0;

                //1.找到本分账单对应的 电费账户 ,
                //1.1  若电费账户余额大于等于0, 表示足够扣电费 , 不需要使用通用账户再进行抵扣.
                TBsAssetAccount account = this.tBsAssetAccountMapper.lookupByBuildCodeAndType(history.getBuildingCode(), BillingEnum.ACCOUNT_TYPE_ELECT.getIntV());
                ...
                if(null == commonAccount){
                    ...
                    if(commonAccount.getAccountBalance() <= 0){
                        ...
                        //1.2.2.2 余额大于0 , 则开始抵扣该电费欠费, 抵扣最大额为commonAccount的余额
                        //先抵扣违约金
                        ...
                        if(!ohs.isEmpty()){
                            ...
                            if(zfLateFee > 0 ) {
                                ...
                                insertStreamList.add(new TBsAssetAccountStream(CommonUtils.getUUID(),commonAccount.getId(),-zfLateFee,
										new Date(),"system","system","通用账户抵扣电费违约金"));
                            ...
                            if(zfLateAmout > 0 ) {
								...
								insertStreamList.add(new TBsAssetAccountStream(CommonUtils.getUUID(),commonAccount.getId(),-zfLateAmout,
										new Date(),"system","system","通用账户抵扣电费历史欠费"));
                        ...
                        if(data != 0){
                            TBsAssetAccountStream commonStream=new TBsAssetAccountStream(commonAccount.getId(), data, StreamEnum.purpose_dk_to_elect.getV());
                            insertStreamList.add(commonStream);  //通用账户的负流水, 抵扣电费
