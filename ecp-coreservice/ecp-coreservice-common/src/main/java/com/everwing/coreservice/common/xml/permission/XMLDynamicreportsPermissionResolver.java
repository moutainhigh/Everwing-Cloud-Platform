 ...
 */
public class XMLDynamicreportsPermissionResolver extends XMLDefaultResolver {
    static Logger logger = LogManager.getLogger(XMLDynamicreportsPermissionResolver.class);
    ...
    private final List<TRightsMenu> parseMenuList = new ArrayList<TRightsMenu>();
    private final List<TRightsResource> parseResourceList = new ArrayList<TRightsResource>();
                ...
                if (ELEMENT_SUBSYSTEM.equals(node.getNodeName())) {   // 解析subsystem节点
                    ...
                    if (!parseMenuMap.containsKey(id)) {  // 忽略已经解析过的节点
                        if (!"*".equals(id) && StringUtils.isNotBlank(id)) {    // id属性值为星号或为空则可以忽略
                            ...
                            TRightsMenu menuEntity = new TRightsMenu();
                    ...
                    if (!parseMenuMap.containsKey(id)) {  // 忽略已经解析过的节点
                        if (!"*".equals(id) && StringUtils.isNotBlank(id)) {    // id属性值为星号或为空则可以忽略
                            ...
                            menuEntity.setMenuPermission(permission);
                            menuEntity.setMenuUrl(url);
                            TRightsMenu tSysMenuParent = parseMenuMap.get(pid) == null ? null : (TRightsMenu) parseMenuMap.get(pid);
                    ...
                    if (!parseResourceMap.containsKey(id)) {  // 忽略已经解析过的节点
                        if (!"*".equals(id) && StringUtils.isNotBlank(id)) {    // id属性值为星号或为空则可以忽略
                            ...
                            TRightsResource resourceEntity = new TRightsResource();
                             ...
                             * 把父节点修改为叶子节点
                             */
                            TRightsMenu tSysMenu = (TRightsMenu) parseMenuMap.get(pid);
