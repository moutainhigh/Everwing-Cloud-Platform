...
import com.everwing.coreservice.common.wy.service.configuration.cmac.service.CmacWyService;
...
@Service("cmacWyService")
public class CmacWyServiceImpl extends Resources implements CmacWyService{

	private static final Logger logger = LogManager.getLogger(CmacWyServiceImpl.class);
	...
	public void invoke(String companyId, TBsProject project) {
		...
		if(CommonUtils.isEmpty(handlers.get(cpId))){
			logger.warn("通用账户自动扣费 -> 扣费项 [物业费] : 当前未找到可执行的服务, 计费停止.");
			return;
		}
		...
		if(CommonUtils.isEmpty(totals)){
			logger.info("通用账户自动扣费 -> 扣费项 [物业费] : 未找到可扣费的本周期物业管理费总账单. 查询参数: {}, 进入下一计费项. ", paramTotal.toString());
			...
			if(CommonUtils.isEmpty(histories)){
				logger.info("通用账户自动扣费 -> 扣费项 [物业费] : 未找到可扣费的本周期分账单. 总账单数据: {}, 进入下一计费项. ", total.toString());
				goNext(cpId, companyId, project);
				return;
			}
			...
			for(TBsChargeBillHistory history : histories){
				...
				TBsAssetAccount account = this.tBsAssetAccountMapper.lookupByBuildCodeAndType(history.getBuildingCode(), BillingEnum.ACCOUNT_TYPE_WY.getIntV());
...
//						insertStreamList.add(new TBsAssetAccountStream(account.getId(), (currBalance >= currBillAmount) ? currBillAmount : currBalance));
						...
						if(data != 0){
							insertStreamList.add(new TBsAssetAccountStream(commonAccount.getId(), data, StreamEnum.purpose_dk_to_wy.getV()));	//通用账户的负流水 抵扣物业管理费
