...
import com.everwing.coreservice.common.wy.service.configuration.cmac.service.CmacElectService;
...
@Service("cmacElectService")
public class CmacElectServiceImpl extends Resources implements CmacElectService{

	private static final Logger logger = LogManager.getLogger(CmacElectServiceImpl.class);
...
//			}
	...
	private void addToOwedHistories(TBsAssetAccount account , List<TBsOwedHistory> insertOhList){
		...
		if(CommonUtils.isEmpty(ohs)){
			...
			double currOwed = Math.abs(account.getAccountBalance());  //当前账户总欠费的绝对值
