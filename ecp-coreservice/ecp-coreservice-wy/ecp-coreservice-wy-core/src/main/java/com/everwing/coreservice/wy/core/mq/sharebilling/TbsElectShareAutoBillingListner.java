...
/**
 * @describe 定时任务平台按公司进行对电费分摊计费的调用
 * @author QHC
 * @date 2017-08-10 17:00:00
 */
@Service("tbsElectShareAutoBillingListner")
public class TbsElectShareAutoBillingListner extends ServiceRrcs implements MessageListener{

	
	private static final Logger logger = Logger.getLogger(TbsElectShareAutoBillingListner.class);
	...
	public void onMessage(Message message) {
		...
		if(null != jsonStr){
			logger.info("*******电费分摊自动计费消息队列监听到任务开始执行S********");
...
//				BillingSupEntity se = ((JSONObject)entity.getData()).toJavaObject(BillingSupEntity.class);
				this.waterElectShareTaskService.doWaterElectShareBillingByCompnay(entity.getCompanyId(),BillingEnum.SHARE_ELECT_TYPE.getIntV());
			}
			logger.info("*******电费分摊自动计费消息队列监听到任务执行E********");
