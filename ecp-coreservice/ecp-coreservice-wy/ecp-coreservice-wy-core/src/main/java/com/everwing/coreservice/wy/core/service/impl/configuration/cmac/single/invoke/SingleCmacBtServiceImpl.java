...
import com.everwing.coreservice.common.wy.service.configuration.cmac.single.servie.SingleCmacBtService;
...
@Service("singleCmacBtService")
public class SingleCmacBtServiceImpl extends Resources implements SingleCmacBtService {

	private static final Logger logger = LogManager.getLogger(SingleCmacBtServiceImpl.class);
	...
	public void invoke(String companyId, TBsProject project,String buildCode) {
		...
		if(CommonUtils.isEmpty(signleHandlers.get(cpId))){
			logger.warn("通用账户自动扣费 -> 扣费项 [本体基金] : 当前未找到可执行的服务, 计费停止.");
			return;
		}
		//找到该项目下的当前的物业总账单
		...
		if(CommonUtils.isEmpty(totals)){
			logger.info("通用账户自动扣费 -> 扣费项 [本体基金] : 未找到可扣费的本周期本体基金总账单. 查询参数: {}, 进入下一计费项. ", paramTotal.toString());
			...
			if(CommonUtils.isEmpty(tbsHistory)){
				logger.info("通用账户自动扣费 -> 扣费项 [本体基金] : 未找到可扣费的本周期分账单. 总账单数据: {}, 进入下一计费项. ", total.toString());
				goNext(cpId, companyId, project,buildCode);
				return;
			}
...
//					insertStreamList.add(new TBsAssetAccountStream(commonAccount.getId(), (currBalance >= currBillAmount) ? -currBillAmount : -currBalance));
					...
					if(data != 0){
						insertStreamList.add(new TBsAssetAccountStream(commonAccount.getId(), data, StreamEnum.purpose_dk_to_bt.getV()));  //通用账户的负流水   扣往本体
