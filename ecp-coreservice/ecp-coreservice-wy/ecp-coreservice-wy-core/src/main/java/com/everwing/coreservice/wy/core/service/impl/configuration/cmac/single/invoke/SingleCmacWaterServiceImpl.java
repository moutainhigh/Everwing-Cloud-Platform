...
import com.everwing.coreservice.common.wy.service.configuration.cmac.single.servie.SingleCmacWaterService;
...
@Service("singleCmacWaterService")
public class SingleCmacWaterServiceImpl extends Resources implements SingleCmacWaterService {

	private static final Logger logger = LogManager.getLogger(SingleCmacWaterServiceImpl.class);
	...
	public void invoke(String companyId, TBsProject project,String buildCode) {
		...
		//扣费开始
		logger.info(CommonUtils.log("水费通用账户扣减开始!"));
		//找到该项目下的当前的水费总账单
		...
		if(CommonUtils.isEmpty(totals)){
			...
			//1.找到本分账单对应的 水费账户 , 
			//1.1  若水费账户余额大于等于0, 表示足够扣取水费 , 不需要使用通用账户再进行抵扣. 
